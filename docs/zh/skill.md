...
目录会被打包并上传到 FC 临时 OSS bucket，然后注册为代码包工具（`createMethod=CODE_PACKAGE`、`artifactType=Code`、`language=python3.12`、`command=python main.py`）。若目录缺少 `main.py`，会注入占位入口文件以保证可部署。
