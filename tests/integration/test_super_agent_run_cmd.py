...
def _patch_all(agent):
    ...
    return (
        patch(
            "agentrun_cli.commands.super_agent.run_cmd.SuperAgentClient",
            return_value=client,
        ),
        patch(
            "agentrun_cli.commands.super_agent.run_cmd.build_sdk_config",
...
class TestRunNonInteractive:
    ...
    def test_run_raw_and_text_only_is_usage_error(self, tmp_path):
        """--raw + --text-only are mutually exclusive."""
        agent = _make_agent()
        (client_p, cfg_p), client = _patch_all(agent)
        state_file = tmp_path / "state.json"
        with (
