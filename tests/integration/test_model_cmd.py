...
def _sdk_patches():
    ...
    mock_model_module.ModelServiceCreateInput = MagicMock(side_effect=lambda **kw: SimpleNamespace(**kw))
    mock_model_module.ModelServiceUpdateInput = MagicMock(side_effect=lambda **kw: SimpleNamespace(**kw))
    mock_model_module.ProviderSettings = MagicMock(side_effect=lambda **kw: SimpleNamespace(**kw))
...
class TestModelCreate:
    ...
    def test_create_with_credential(self):
        mock_mod = _sdk_patches()
        mock_mod.ModelService.create.return_value = _make_mock_service(credential_name="my-cred")
        with _patch_sdk(mock_mod):
            runner = CliRunner()
            result = runner.invoke(cli, [
                "model", "create",
...
class TestModelUpdate:
    ...
    def test_update_with_flags(self):
        mock_mod = _sdk_patches()
        mock_mod.ModelService.update_by_name.return_value = _make_mock_service(description="updated")
        with _patch_sdk(mock_mod):
            runner = CliRunner()
            result = runner.invoke(cli, [
                "model", "update",
    ...
    def test_update_description_only_no_provider_settings(self):
        mock_mod = _sdk_patches()
        mock_mod.ModelService.update_by_name.return_value = _make_mock_service(description="desc only")
        with _patch_sdk(mock_mod):
            runner = CliRunner()
            result = runner.invoke(cli, [
                "model", "update",
